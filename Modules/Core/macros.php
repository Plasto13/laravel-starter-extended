...
Form::macro('normalDate', function ($name, $title, ViewErrorBag $errors, $object = null, array $options = []) {
    $options = array_merge(['class' => 'form-control', 'placeholder' => $title], $options);
    ...
    $string .= Form::date($name, old($name, $currentData), $options);
    $string .= $errors->first($name, '<span class="help-block">:message</span>');
    $string .= '</div>';
...
Form::macro('normalDateTime', function ($name, $title, ViewErrorBag $errors, $object = null, array $options = []) {
    $options = array_merge(['class' => 'form-control', 'placeholder' => $title], $options);
    ...
    if (is_object($object)) {
        $currentData = isset($object->{$name}) ? $object->{$name}->tz(setting('base::timezone'))->format(setting('base::datetimeformat')) : '';
    } else {
        $currentData = null;
    }
...
Form::macro('normalOldTextarea', function ($name, $title, ViewErrorBag $errors, $object = null, array $options = []) {
