<div class="wrapper-filter">
    ...
    <input type="hidden" class="filter-data-url" value="{{ $route }}">
            ...
            <div class="ui-select-wrapper">
                <select name="filter_columns[]" class="ui-select filter-column-key">
                    ...
                    @foreach($columns as $column_key => $column)
                        <option data-type="{{$column['type']}}" value="{{ $column_key }}">{{ $column['title'] }}</option>
                    @endforeach
                </select>
    ...
    {{ Form::open(['method' => 'GET', 'class' => 'filter-form']) }}
                ...
                <div class="filter-item form-filter @if ($loop->first) filter-item-default @endif">
                    <div class="ui-select-wrapper">
                        <select name="filter_columns[]" class="ui-select filter-column-key">
                            <option value="">{{ trans('core::table.general.select_field') }}</option>
                            @foreach($columns as $column_key => $column)
                                <option data-type="{{$column['type']}}" value="{{ $column_key }}" @if ($filter_item['column'] == $column_key) selected @endif>{{ $column['title'] }}</option>
                            @endforeach
                        </select>
                        <svg class="svg-next-icon svg-next-icon-size-16">
                            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#select-chevron"></use>
        ...
        <div style="margin-top: 10px;">
            ...
            <a href="{{ URL::current() }}" class="btn btn-info">{{ trans('core::table.general.reset') }}</a>
            <button type="submit" class="btn btn-primary btn-apply">{{ trans('core::table.general.apply') }}</button>
            <button class="btn btn-primary btn-in-trash-filter">{{ trans('core::table.general.in trash') }}</button>
        </div>
